-- TESTE: Alterar regra de avaliação para 30 segundos
-- Execute este SQL no Supabase SQL Editor para testar

-- 1. Atualizar função de verificação de avaliação
...
BEGIN
  ...
  -- TESTE: Mudar de 20 minutos para 1 minuto
  RETURN time_together >= 1;
END;
$$ LANGUAGE plpgsql;

-- 2. Atualizar trigger para usar 30 segundos
...
BEGIN
  IF NEW.left_at IS NOT NULL AND OLD.left_at IS NULL THEN
    ...
    -- TESTE: Mudar de 20 minutos para 1 minuto
    NEW.can_evaluate := NEW.total_time_minutes >= 1;
  END IF;
  RETURN NEW;
END;
$$ LANGUAGE plpgsql;
